...
export default function TopBar({
  ...
  return (
    <header className={`flex items-center justify-between ${isLobby ? 'h-20 bg-black' : `h-16 ${solidBg ? 'bg-[#0d0d1e]' : 'bg-[#0d0d1e]/30 backdrop-blur-md'}`} px-4 border-b-4 border-black flex-shrink-0 relative z-10 select-none comic-halftone`}>
      ...
      <div className="flex items-center gap-4 flex-1">
        ...
        {!isLobby && onNavigate && (
          <div className="flex items-center gap-2 border-2 border-black bg-black/40 rounded-lg p-1 shadow-[2px_2px_0px_#000]">
            <button
      ...
      {/* Center Section */}
      <div className="flex flex-col items-center justify-start h-full flex-shrink-0 relative w-64">
        {/* Center Mission Time Digital Panel */}
        {!isLobby ? (
          <div className="self-start flex flex-col items-center bg-[#05050a] border-x-4 border-b-4 border-black px-8 py-1.5 shadow-[0px_4px_0px_0px_rgba(0,0,0,0.15)] rounded-b-xl z-20">
      ...
      <div className="flex items-center justify-end gap-3 flex-1">
        {/* Team Info */}
        <div className="flex items-center gap-2 bg-black/40 border-2 border-black rounded-lg px-2 py-1 shadow-[2px_2px_0px_0px_#000]">
        ...
        {!hideSubmit && (
          <button 
            ...
            className="px-4 py-1.5 bg-red-600 hover:bg-red-500 text-white font-bold rounded-lg border-2 border-black shadow-[2px_2px_0px_0px_#000] active:translate-y-0.5 active:translate-x-0.5 active:shadow-[0px_0px_0px_0px_#000] transition-all comic-halftone whitespace-nowrap text-sm"
          >
            SUBMIT TEST
          </button>
        )}
